...
- Links to all other docs files (repo-overview, architecture, code-map)
- Links to spec files (main spec + each feature/module spec)
- A "KIRO Files" section — but **only if** `.kiro/steering/` or `.kiro/specs/` actually exist in the repository. If they exist, list only the files that are relevant to understanding the repository (e.g., steering files about project overview, architecture, conventions). Omit this section entirely if neither folder exists or if no relevant files are found inside them.
...
- [Main Spec](../specs/service-spec.md) — Responsibilities, interfaces, dependencies
- [Feature/Module: <name>](../specs/features/<name>-spec.md)
<!-- Add a line for each feature / module / resource group -->

<!-- Include this section ONLY if .kiro/steering/ or .kiro/specs/ exist in the repo AND contain relevant files. Omit entirely otherwise. -->
## KIRO Files

- .kiro/steering/<relevant-file>.md — <brief description>
- .kiro/specs/<relevant-file>.md — <brief description>
<!-- List only files relevant to understanding the repo. Remove any line where the folder doesn't exist. -->
...
- Links to feature/module specs under `features/`
- Reference to generated specs (`.kiro/specs/`, `_bmad-output/`) if those folders exist
